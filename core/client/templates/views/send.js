...
var getDataField = function(){
    ...
    if(selectedToken && selectedToken !== 'iron') {
...
Template['views_send'].onCreated(function(){
    ...
    if(FlowRouter.getRouteName() === 'deployContract') {
        ...
        TemplateVar.set('selectedToken', 'iron');

    // Send funds
    } else {
        TemplateVar.set('selectedAction', 'send-funds');
        TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'iron');
    }
    ...
    template.autorun(function(c){
        ...
        if(!c.firstRun && TemplateVar.get('selectedToken') === 'iron') {
...
Template['views_send'].helpers({
    ...
    */
    'total': function(iron){
        ...
        if (TemplateVar.get('selectedToken') === 'iron') {
    ...
    */
    ...
    /**
    Returns the total amount - the fee paid to send all iron/coins out of the account
    ...
    */
    'formattedCoinBalance': function(e){
        ...
        return (this.balances && Number(this.balances[selectedAccount._id]) > 0)
            ? Helpers.formatNumberByDecimals(this.balances[selectedAccount._id], this.decimals)+' IRON'
            : false;
    },
    /**
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'iron'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'iron'))
                return GlobalNotification.warning({
