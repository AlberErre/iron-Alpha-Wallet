...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
    ...
    if((unit === 'usd' || unit === 'eur' || unit === 'btc') &&
       exchangeRates && exchangeRates[unit]) {
        ...
        var price = new BigNumber(String(web3.fromWei(value, 'iron')), 10).times(exchangeRates[unit].price);
        return EthTools.formatNumber(price, format);
    } else {
        return EthTools.formatBalance(value, format + '[0000000000000000]');
    }
};
